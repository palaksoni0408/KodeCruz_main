...
var import_client_side = require("../helpers/vault/client-side.js");
...
var StackServerInterface = class extends import_client_interface.StackClientInterface {
  ...
  async listServerNotificationCategories(userId) {
    const response = await this.sendServerRequest(
      import_urls.urlString`/emails/notification-preference/${userId}`,
      {
        method: "GET"
      },
      null
  ...
  async setServerNotificationsEnabled(userId, notificationCategoryId, enabled) {
    await this.sendServerRequest(
      import_urls.urlString`/emails/notification-preference/${userId}/${notificationCategoryId}`,
      {
        method: "PATCH",
        headers: {
          "content-type": "application/json"
  ...
  async updateServerOAuthProvider(userId, providerId, data) {
    const response = await this.sendServerRequest(
      import_urls.urlString`/oauth-providers/${userId}/${providerId}`,
      {
        method: "PATCH",
        headers: {
          "content-type": "application/json"
  ...
  async grantProduct(options) {
    if (!options.productId && !options.product) {
      throw new import_errors.StackAssertionError("grantProduct requires either productId or product");
    }
    if (options.productId && options.product) {
      throw new import_errors.StackAssertionError("grantProduct should not receive both productId and product");
    }
    const body = (0, import_objects.filterUndefined)({
      product_id: options.productId,
      product_inline: options.product,
      quantity: options.quantity
    });
    await this.sendServerRequest(
      import_urls.urlString`/payments/products/${options.customerType}/${options.customerId}`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  async getDataVaultStoreValue(secret, storeId, key) {
    const hashedKey = await (0, import_client_side.hashKey)(secret, key);
    const response = await this.sendServerRequestAndCatchKnownError(
      ...
      null,
      [import_known_errors.KnownErrors.DataVaultStoreHashedKeyDoesNotExist]
    );
    if (response.status === "error") {
      if (import_known_errors.KnownErrors.DataVaultStoreHashedKeyDoesNotExist.isInstance(response.error)) {
        return null;
      } else {
        throw new import_errors.StackAssertionError("Unexpected uncaught error", { cause: response.error });
      }
    }
    const json = await response.data.json();
    const encryptedValue = json.encrypted_value;
    if (typeof encryptedValue !== "string") throw new import_errors.StackAssertionError("encrypted_value is not a string", { type: typeof encryptedValue });
    return await (0, import_client_side.decryptValue)(secret, key, encryptedValue);
  }
  async setDataVaultStoreValue(secret, storeId, key, value) {
    const hashedKey = await (0, import_client_side.hashKey)(secret, key);
    const encryptedValue = await (0, import_client_side.encryptValue)(secret, key, value);
    await this.sendServerRequest(
