...
__export(schema_fields_exports, {
  ...
  oauthProviderProviderConfigIdSchema: () => oauthProviderProviderConfigIdSchema,
  ...
  priceOrIncludeByDefaultSchema: () => priceOrIncludeByDefaultSchema,
  ...
  productClientMetadataSchema: () => productClientMetadataSchema,
  productClientReadOnlyMetadataSchema: () => productClientReadOnlyMetadataSchema,
  productPriceSchema: () => productPriceSchema,
  productSchema: () => productSchema,
  productSchemaWithMetadata: () => productSchemaWithMetadata,
  productServerMetadataSchema: () => productServerMetadataSchema,
  profileImageUrlSchema: () => profileImageUrlSchema,
  projectAllowLocalhostSchema: () => projectAllowLocalhostSchema,
  projectBranchIdSchema: () => projectBranchIdSchema,
  ...
  projectLogoDarkModeUrlSchema: () => projectLogoDarkModeUrlSchema,
  projectLogoFullDarkModeUrlSchema: () => projectLogoFullDarkModeUrlSchema,
  projectLogoFullUrlSchema: () => projectLogoFullUrlSchema,
  ...
  wildcardProtocolAndDomainSchema: () => wildcardProtocolAndDomainSchema,
  wildcardUrlSchema: () => wildcardUrlSchema,
  yupArray: () => yupArray,
...
yup.addMethod(yup.Schema, "getNested", function(path) {
  if (!path.match(/^[a-zA-Z0-9_$:-]*$/)) throw new import_errors.StackAssertionError(`yupSchema.getNested can currently only be used with alphanumeric keys, underscores, dollar signs, colons, and hyphens. Fix this in the future. Provided key: ${JSON.stringify(path)}`);
...
var wildcardUrlSchema = yupString().test({
  ...
  test: (value) => {
    if (value == null) return true;
    if (!value.includes("*")) {
      return (0, import_urls.isValidUrl)(value);
    }
    try {
      ...
      return (0, import_urls.isValidHostnameWithWildcards)(hostPattern);
    } catch (e) {
      return false;
    }
  }
...
var validateHasAtLeastOneSupportedCurrency = (value, context) => {
  if (!value) return true;
  const currencies = Object.keys(value).filter((key) => import_currency_constants.SUPPORTED_CURRENCIES.some((c) => c.code === key));
...
var productPriceSchema = yupObject({
  ...(0, import_objects.typedFromEntries)(import_currency_constants.SUPPORTED_CURRENCIES.map((currency) => [currency.code, moneyAmountSchema(currency).optional()])),
...
var neonAuthorizationHeaderSchema = basicAuthorizationHeaderSchema.test("is-authorization-header", "Invalid client_id:client_secret values; did you use the correct values for the integration?", (value) => {
  if (!value) return true;
  const [clientId, clientSecret] = (0, import_http.decodeBasicAuthorizationHeader)(value) ?? (0, import_errors.throwErr)(`Authz header invalid? This should've been validated by basicAuthorizationHeaderSchema: ${value}`);
